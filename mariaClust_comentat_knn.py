	...
	while(nr_clusters_agg > nr_final_clusters):
		...
		for cluster_point in cluster_points[(newCluster[0], newCluster[1])]:
			if(cluster_point in intermediary_centroids):
				intermediary_centroids = filter(lambda a: a != cluster_point, intermediary_centroids)
		
		if(value_a in intermediary_centroids):
			intermediary_centroids = filter(lambda a: a != value_a, intermediary_centroids)

		if(value_b in intermediary_centroids):
			intermediary_centroids = filter(lambda a: a != value_b, intermediary_centroids)
...
def get_closestk_neigh(point, dataset_k, k, id_point):
	...
	while(k>0):
		...
		#print("k = "+str(k)+" id_neigh "+ str(neigh_id) +" neigh "+str(neigh))
		k = k-1
	#print("---end neigh")
	#print("len dataset_k later "+str(len(dataset_k)))
	#print("len neighid "+str(len(neigh_ids)))

	#iau candidatii neparcursi

	'''candidates = list()

	for id_point in range(len(dataset_k)):
		point_cand = dataset_k[id_point]
		if(point_cand[4]==-1 and (point_cand not in deja_parsati) ):
			candidates.append([point_cand, id_point])

	#print("dataset_k "+str(dataset_k))
	k = 1
	if(len(candidates)>0):
		while(k>0):
			minDist = 99999
			neigh_id = 0
			neigh = 0
			for id_point_k in range(len(candidates)):
				point_k = candidates[id_point_k][0]
				if(point_k not in deja_parsati):
					dist = DistFunc(point, point_k)
					if(dist < minDist and dist > 0):
						minDist = dist
						neigh_id = candidates[id_point_k][1]
						neigh = candidates[id_point_k][0]
			

			#daca minDist este cam cat celelalte minDist-uri il pastrez, daca e vizibil prea mare il arunc
			avgDist = 1.5*sum(distances)/len(distances)
			#print("minDist "+str(minDist)+" medie "+str(avgDist))
			if(minDist <= avgDist):
				#print("DA!")
				neigh_ids.append(neigh_id)
				deja_parsati.append(neigh)
	...
	'''for pixel_id in range(len(dataset_k)):
		pixel = dataset_k_aux[pixel_id]
		if(pixel_id in neigh_ids):
	...
	#print("len neighid fin "+str(len(neigh_ids)))
	return neigh_ids

def expand_knn(point_id, k):
	'''
	...
	neigh_ids = get_closestk_neigh(point, pixels_partition_clusters, k, point_id)
...
pixels_per_bin, bins = np.histogram(pdf, bins=2) #!!!! numarul de binuri e nr clustere-1
...
for k in partition_dict:
	...
	just_points = list()
	nrAnchors = int(math.ceil(0.25*len(pixels_partition)))
	knn = int(math.ceil(0.03*len(pixels_partition))) #0.35 pt pathbased, 0.3 pentru jain
	print("nr vecini = "+str(knn))

	for pixel in pixels_partition:
		pixels_partition_clusters.append([pixel[0], pixel[1], -1, pdf[pixel[2]], -1]) #id cluster, pdf de idx_point, deja_parsat
		just_points.append([pixel[0], pixel[1]])

	kmeans = KMeans(n_clusters=nrAnchors, random_state=0).fit(just_points)
	anchor_points = kmeans.cluster_centers_

	for pixel in anchor_points:
		pixels_partition_clusters.append([pixel[0], pixel[1], -1, -1, -1]) #id_cluster, probabilitate, deja_parsat

	#anchor_points = anchor_points[0:2]

	for pixel in pixels_partition_clusters:
		pixel_helper = [pixel[0], pixel[1]]
		if(pixel_helper in anchor_points):
			plt.scatter(pixel[0], pixel[1], color="r")
		else:
			plt.scatter(pixel[0], pixel[1], color="g")
	plt.show()
	'''for pixel_id in range(len(pixels_partition_clusters)):
		if(pixels_partition_clusters[pixel_id][2]==-1):
			...
			neigh_ids = get_closestk_neigh(pixel, pixels_partition_clusters, knn)
			clusters[id_cluster].append(pixel)
			pixels_partition_clusters[pixel_id][2] = id_cluster
			for neigh_id in neigh_ids:
				expand_knn(neigh_id, knn)
	...
	for pixel_id in range(len(pixels_partition_clusters)):
		...
		pixel_helper = [pixel[0], pixel[1]]
		if(pixel_helper in anchor_points):
			neigh_ids = get_closestk_neigh(pixel, pixels_partition_clusters, knn, pixel_id)
			print("neigh ids "+str(neigh_ids))
			clusters[id_cluster].append(pixel)
			pixels_partition_clusters[pixel_id][2] = id_cluster
			for neigh_id in neigh_ids:
	...
	#TO DO daca am spart in prea multe bucatele le unesc
	de_unit = list()
	for part_id_i in range(len(inner_partitions_filtered)-1):
		avg_sum = 0
		smallest_between = 0
		#avg_i = calculate_average_cluster(inner_partitions_filtered[part_id_i])
		for part_id_j in range(part_id_i+1, len(inner_partitions_filtered)):
			#avg_j = calculate_average_cluster(inner_partitions_filtered[part_id_j])
			#avg_sum = avg_i + avg_j
			#print("i "+str(part_id_i)+" j"+str(part_id_j))
			smallest_between = calculate_smallest_pairwise(inner_partitions_filtered[part_id_i], inner_partitions_filtered[part_id_j])
			if (smallest_between <= 0):
				de_unit.append([part_id_i, part_id_j])

	for chei_de_unit in de_unit:
		aux_unit = inner_partitions_filtered[chei_de_unit[1]]
		del inner_partitions_filtered[chei_de_unit[1]]

		for pixel in aux_unit:
			inner_partitions_filtered[chei_de_unit[0]].append(pixel)
...
'''intermediary_centroids, cluster_points = agglomerative_clustering2(final_partitions, 10) #paramateri: partitiile rezultate, numarul de clustere
...
intermediary_centroids, cluster_points = agglomerative_clustering2(final_partitions, 2) #paramateri: partitiile rezultate, numarul de clustere
print(intermediary_centroids)
print("==============================")
#print(cluster_points)
